...
### 4. 导航菜单与权限同步 (Menu & Permission Sync)
新增导航菜单项时，必须同步完成以下步骤：
1. **`templates/base.html`**: 在 `<nav>` 中添加 `<li data-perm="xxx">` 菜单项
2. **数据库 `system_settings` 表**: 添加 `perm_xxx` 记录，设置各角色权限
3. **验证**: 确保管理员/计划员/班组角色都能按预期看到/隐藏菜单

常见错误：将新菜单嵌套在已有 `<li data-perm="...">` 内部，导致权限被父级覆盖。
