...
class OpenAICompatibleClient(object):
    ...
    def chat(self, messages, temperature=0.2):
        ...
        try:
            ...
            raise AIClientError("调用模型失败: " + str(exc))
        if resp.status_code >= 400:
            raise AIClientError("调用模型失败: HTTP %s %s" % (resp.status_code, resp.text[:300]))
        data = resp.json()
        try:
            ...
            if reasoning is not None:
                return reasoning
            raise AIClientError("模型返回空响应")
        except Exception:
            raise AIClientError("模型返回格式无法识别")
