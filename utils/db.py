...
def init_database():
    ...
    try:
        ...
        if "duplicate column" not in str(e).lower():
            raise
    try:
        c.execute("ALTER TABLE standard_hours ADD COLUMN weld_count REAL DEFAULT 0")
    except sqlite3.OperationalError as e:
        if "duplicate column" not in str(e).lower():
            raise
    ...
    c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('ai_chroma_mode', 'http')")
    c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('ai_chroma_persist_dir', 'data/chroma')")
    c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('ai_chroma_host', 'localhost')")
    c.execute("INSERT OR IGNORE INTO system_settings (key, value) VALUES ('ai_chroma_port', '8000')")
